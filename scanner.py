#!/usr/bin/env python
import re, urllib2, urllib, argparse, datetime, time
...
class Xss(object):
	...
	writeintofile = ""
	proxy = None # It's pretty obvious that this variable is used for proxy request
	total = 0
	...
	def get_forms(self):
		...
		forms = re.findall("""<form.*?action=['|"](.*?)['|"].*?method=['|"](.*?)['|"].*?>(.*?)</form>""", data, re.DOTALL) # Parse forms
	...
	def send_payload(self, url, data, form):
		...
		temp_url = url
		if form[0][0] == "/":
			temp_url = re.findall("^(http.*?\/\/.*?)/.*$", temp_url)[0] + form[0]
		...
		if self.injection in html:
			self.vulnerabilities.append([urllib.unquote(temp_url).replace("+", " "), data, form[1].upper()])
...
def main():
	...
	parser.add_argument('-l','--logfile', help='Optional log file', required=False)
	parser.add_argument('-p','--proxy', help='Optional proxy service', required=False)
